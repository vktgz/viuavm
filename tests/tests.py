...
class AssemblerErrorTests(unittest.TestCase):
    ...
    def testNegativeRegisterIndex(self):
        name = 'negative_register_index.asm'
        assembly_path = os.path.join(self.PATH, name)
        compiled_path = os.path.join(COMPILED_SAMPLES_PATH, '{0}_{1}.bin'.format(self.PATH[2:].replace('/', '_'), name))
        output, error, exit_code = assemble(assembly_path, compiled_path, okcodes=(1,0))
        self.assertEqual(1, exit_code)
        self.assertEqual("./sample/asm/errors/negative_register_index.asm: error in function main/0 (at instruction 4): invalid register index (cannot be negative): -1", output.strip())
