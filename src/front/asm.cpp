...
string sha512(const unsigned char* s, unsigned len) {
    ostringstream oss;

    const unsigned int HASH_LENGTH = 32;
    unsigned char hash[HASH_LENGTH];
    SHA256(s, len, hash);
    for (unsigned i = 0; i < HASH_LENGTH; ++i) { oss << hex << (unsigned int)(hash[i]); }

    return oss.str();
}
string sha512(const char* s, unsigned len) {
    return sha512((unsigned char*)s, len);
}
...
int generate(const string& filename, string& compilename, const vector<string>& commandline_given_links) {
    ...
    for (string name : function_names) {
        ...
        if (DEBUG) {
            cout << "[asm] debug: hash of function \"" << name << "\" -> ";
            cout << sha512(btcode, func.size());
            cout << endl;
        }
