...
viua::internals::types::byte* viua::process::Process::opress(viua::internals::types::byte* addr) {
    ...
    switch (static_cast<viua::internals::RegisterSets>(to_register_set)) {
        ...
        case viua::internals::RegisterSets::LOCAL:
            currently_used_register_set = current_stack()->back()->local_register_set.get();
            break;
        case viua::internals::RegisterSets::STATIC:
            ensureStaticRegisters(current_stack()->back()->function_name);
            currently_used_register_set = static_registers.at(current_stack()->back()->function_name).get();
            break;
        default:
