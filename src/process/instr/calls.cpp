...
viua::internals::types::byte* viua::process::Process::opparam(viua::internals::types::byte* addr) {
    ...
    if (parameter_no_operand_index >= current_stack()->frame_new->arguments->size()) {
    ...
    current_stack()->frame_new->arguments->set(parameter_no_operand_index, source->copy());
    current_stack()->frame_new->arguments->clear(parameter_no_operand_index);
...
viua::internals::types::byte* viua::process::Process::oppamv(viua::internals::types::byte* addr) {
    ...
    current_stack()->frame_new->arguments->set(parameter_no_operand_index, currently_used_register_set->pop(source));
    current_stack()->frame_new->arguments->clear(parameter_no_operand_index);
    current_stack()->frame_new->arguments->flag(parameter_no_operand_index, MOVED);
...
viua::internals::types::byte* viua::process::Process::oparg(viua::internals::types::byte* addr) {
    ...
    if (parameter_no_operand_index >= current_stack()->back()->arguments->size()) {
        ostringstream oss;
    ...
    if (current_stack()->back()->arguments->isflagged(parameter_no_operand_index, MOVED)) {
        argument = current_stack()->back()->arguments->pop(parameter_no_operand_index);
    } else {
        argument = current_stack()->back()->arguments->get(parameter_no_operand_index)->copy();
    }
...
viua::internals::types::byte* viua::process::Process::opargc(viua::internals::types::byte* addr) {
    ...
    *target = unique_ptr<viua::types::Type>{new viua::types::Integer(static_cast<int>(current_stack()->back()->arguments->size()))};
...
viua::internals::types::byte* viua::process::Process::optailcall(viua::internals::types::byte* addr) {
    ...
    Frame *last_frame = current_stack()->back().get();

    last_frame->arguments = std::move(current_stack()->frame_new->arguments);
...
viua::internals::types::byte* viua::process::Process::opreturn(viua::internals::types::byte* addr) {
    ...
    addr = current_stack()->back()->ret_address();

    unique_ptr<viua::types::Type> returned;
    viua::kernel::Register* return_register = current_stack()->back()->return_register;
    ...
    if (current_stack()->size() > 0) {
        adjustJumpBaseFor(current_stack()->back()->function_name);
    }
